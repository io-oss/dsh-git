...
._git_modalHint{font-size:12px;color:var(--dsw-alias-label-secondary);margin-bottom:12px}
...
`;
    ...
    function ChangesPanel({ staged, unstaged, untracked, onSelectFile, selectedFile, excluded, onToggleExcluded }) {
      ...
      function renderItem(file, area) {
        ...
        return react_jsx_runtime.jsx("div", {
          ...
          onClick: () => onSelectFile(file.path, area),
          children: react_jsx_runtime.jsxs(react_jsx_runtime.Fragment, {
            children: [
      ...
      function Section({ title, items, open, onToggle, count, area }) {
        ...
        return react_jsx_runtime.jsxs("div", { className: "_git_section", children: [
          ...
          open && react_jsx_runtime.jsx("div", { className: "_git_sectionContent", children: items.map((item) => renderItem(item, area)) }),
        ] });
      }

      return react_jsx_runtime.jsxs(react_jsx_runtime.Fragment, {
        children: [
          react_jsx_runtime.jsx(Section, { title: "已暂存 (Staged)", area: "staged", items: staged, open: stagedOpen, onToggle: () => setStagedOpen(!stagedOpen), count: staged.length }),
          react_jsx_runtime.jsx(Section, { title: "未暂存 (Unstaged)", area: "unstaged", items: unstaged, open: unstagedOpen, onToggle: () => setUnstagedOpen(!unstagedOpen), count: unstaged.length }),
          react_jsx_runtime.jsx(Section, { title: "未跟踪 (Untracked)", area: "untracked", items: untracked.map((p) => ({ path: p, status: "??" })), open: untrackedOpen, onToggle: () => setUntrackedOpen(!untrackedOpen), count: untracked.length }),
        ],
      });
    }
    ...
    function BranchModal({ open, onClose, onCreate, baseName }) {
      const [name, setName] = react.useState("");
      if (!open) return null;
      return react_jsx_runtime.jsx("div", { className: "_git_modalOverlay", onClick: onClose, children: react_jsx_runtime.jsxs("div", { className: "_git_modal", onClick: (e) => e.stopPropagation(), children: [
        react_jsx_runtime.jsx("div", { className: "_git_modalTitle", children: "新建分支并切换" }),
        react_jsx_runtime.jsx("div", { className: "_git_modalHint", children: `基点：${baseName || "当前 HEAD"}` }),
        react_jsx_runtime.jsx("input", {
    ...
    function GitView({ workspacePath, viewRequest, completeViewRequest, renderSlot, t }) {
      ...
      const handleSelectFile = react.useCallback(async (filePath, area) => {
        setSelectedFile(filePath);
        if (!git || !selectedRepo) return;
        // area 由列表分区直接给出（staged/unstaged/untracked），不再靠状态字母猜测
        try { const r = await git.diff({ repoPath: selectedRepo, filePath, area }); setDiffText(r.ok ? r.value : ""); }
      ...
      const doCreateBranch = react.useCallback(async (name) => {
        ...
        setBusyBranch(true);
        try {
          // 基点：下拉选中的非当前分支 → startPoint；否则基于当前 HEAD
          const startPoint = targetBranch && targetBranch !== currentName ? targetBranch : undefined;
          const r = await git.createBranch({ repoPath: selectedRepo, branch: name, ...(startPoint ? { startPoint } : {}) });
          if (r.ok) {
      ...
      return react_jsx_runtime.jsxs("div", { className: "_git_root", children: [
        ...
        react_jsx_runtime.jsx(AddRepoModal, { open: addRepoOpen, onClose: () => setAddRepoOpen(false), onConfirm: handleAddRepo }),
        react_jsx_runtime.jsx(BranchModal, { open: branchModalOpen, onClose: () => setBranchModalOpen(false), onCreate: doCreateBranch, baseName: targetBranch && targetBranch !== currentName ? targetBranch : (currentName || "") }),
        react_jsx_runtime.jsx(Toast, { toast }),
      ] });
    }
