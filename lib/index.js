...
/** 轻量二进制嗅探：嗅探窗口内出现 NUL 字节即视为二进制 */
function looksBinary(buf) {
  const max = Math.min(buf.length, BINARY_SNIFF_BYTES);
...
class GitService {
  ...
  /**
   * diff 区域归一化：显式 area 优先，兼容旧的 staged 布尔。
   * - staged    ：index ↔ HEAD（git diff --cached）
   * - unstaged  ：worktree ↔ index（git diff）
   * - untracked ：HEAD/index 均无此文件，单独处理
   */
  diffArea(request) {
    if (request.area === "staged" || request.area === "unstaged" || request.area === "untracked") {
      return request.area;
    }
    return request.staged === true ? "staged" : "unstaged";
  }

  /** 单文件差异。area 缺省时按旧的 staged 布尔回退（staged→已暂存，否则未暂存）。 */
  async diff(request) {
    const { repoPath, filePath } = request;
    if (typeof filePath !== "string" || filePath === "") return "";
    const area = this.diffArea(request);
    if (area === "untracked") return this.untrackedFileDiff(repoPath, filePath);
    const cached = area === "staged" ? "--cached " : "";
  ...
  /**
   * 未跟踪文件的"改动"预览。
   * git diff 默认不展示未跟踪文件内容，因此这里先校验该路径确为当前仓库的未跟踪
   * 文件（顺带防路径穿越），再读取工作区文件，用
   * `git diff --no-index -- /dev/null <file>` 合成"整文件新增"的统一 diff；
   * 二进制或超过 DIFF_MAX_BYTES 时返回友好提示文本（客户端按普通文本渲染）。
   */
  untrackedFileDiff(repoPath, filePath) {
    const listed = safeGit(repoPath, `ls-files --others --exclude-standard -- "${filePath}"`);
    if (!listed.ok || listed.value === "") return "";

    let buf;
    try {
      buf = readFileSync(join(repoPath, filePath));
    } catch {
      return "";
    }
    if (buf.length > DIFF_MAX_BYTES) {
      const mb = (buf.length / 1024 / 1024).toFixed(2);
      return `文件过大（${mb} MB，预览上限 1 MB），已跳过差异对比。`;
    }
    if (buf.length === 0) return "（空文件，无内容可对比）";
    if (looksBinary(buf)) return "二进制文件，已跳过差异对比。";

    // --no-index 在存在差异时退出码为 1（execSync 会抛错），stdout 仍是有效 diff，故用 || true 保留输出
    const result = safeGit(repoPath, `diff --no-index -- /dev/null "${filePath}" || true`);
    return result.ok ? result.value : "";
  }
